...
angular.module("cryptoApp")
    .controller("newInvestmentCtrl", function ($rootScope, $scope, db) {
        ...
        let cryptoRowTemplate = document.querySelector("script[type='cryptoRowTemplate']").innerHTML;
        // crypto rows data
        ...
        $scope.findUser = () => {
            ...
            db.getOneUser($scope.userIdentificationData)
            .then((res) => {
        ...
        $scope.updateCryptoData = (newCryptoName, newCryptoPrice) => {
            ...
            if (newCryptoName !== null) {
                ...
                // delete action
                let selectedCoinName = event.target.parentElement.parentElement.children[0].innerHTML;
                for (let i = 0; i < $scope.cryptoRowsData.length; i++) {
        ...
        function renderCryptoRows() {
            ...
            let cryptoRowsContainer = document.querySelector("div.cryptoRowsContainer");    
            let inside = "";                                            
            // render existing rows
            for (let i = 0; i < $scope.cryptoRowsData.length; i++) {

                inside += cryptoRowTemplate.replace("$$cryptoRowIndex$$", i)
                                            .replace("$$cryptoName$$", $scope.cryptoRowsData[i].name)
                                            .replace("$$totalValue$$", $scope.cryptoRowsData[i].totalValue + "$");            
            }
        ...
        function updateCoinAmount() {
            ...
            let selectedCoinName = this.parentElement.previousElementSibling.innerHTML;
            let amount = this.value;
            // update $scope.cryptoRowsData
            for (let i = 0; i < $scope.cryptoRowsData.length; i++) {
                if ($scope.cryptoRowsData[i].name == selectedCoinName) {
                    $scope.cryptoRowsData[i].amount = amount;
                    $scope.cryptoRowsData[i].totalValue = Number($scope.cryptoRowsData[i].amount) * Number($scope.cryptoRowsData[i].price);
                }
            }
            renderCryptoRows();
        }
